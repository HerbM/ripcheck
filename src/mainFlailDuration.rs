...
async fn test_tcp_portlist(host: &String, name: &String, portlist: &Vec<String>, wait: Duration, hostwidth: usize) {
  let mut tasks = vec![];
  for p in portlist.clone() {
    let h = host.clone();
    let delay = rand::thread_rng().gen_range(1000, 2000);
    let task  = delay_for(
      Duration::from_millis(0)).then(async move |_| {
  ...
  for t in tasks {
    let result = format!("{}", t.await.unwrap().unwrap_or(false));
    results.push(result);
  }
